...
import { and, eq, or, inArray, isNull, asc, sql } from "drizzle-orm";
...
export async function updateProject(
  userId,
  projectId,
  { name, config, welcomeMarkdown, includeClosedIssues, adrRepoId, adrPath } = {}
) {
...
export async function claimEnvironment(
  ...
  if (!row) {
    // Either it does not exist, or somebody already holds it. Distinguish the
    // two so the UI can say something useful.
    const [existing] = await loadEnvironments(projectId, environmentId);
    if (!existing) throw new NotFoundError("Environment not found.");
    if (existing.claim?.userId === userId) return existing;
    throw new ValidationError([
      {
        field: "claim",
...
export async function releaseEnvironment(userId, projectId, environmentId) {
  ...
  await db
    .update(projectEnvironments)
    .set({ claimedBy: null, claimedAt: null, claimNote: null })
    .where(
      and(
        eq(projectEnvironments.id, environmentId),
        eq(projectEnvironments.projectId, projectId)
...
export async function refreshIssues(userId, projectId) {
  ...
  const [row] = await db
    .select({ includeClosed: projects.includeClosedIssues, config: projects.config })
    .from(projects)
    .where(eq(projects.id, projectId))
    .limit(1);
...
async function loadEnvironments(projectId, environmentId = null) {
  ...
  return rows.map((r) => toEnvironmentView(r, r.claimedBy ? r : null));
}
