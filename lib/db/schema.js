...
export const projects = pgTable(
  "projects",
  {
    ...
    // Agent tab. Credentials are per-USER (see llmCredentials) because a
    // Copilot seat is licensed to a person; only the enablement and model
    // choice are project-level.
...
/**
 * Per-user LLM credentials, encrypted at rest with the same AES-256-GCM key as
 * repository PATs (lib/crypto.js).
 *
 * Deliberately keyed by USER, not project. A GitHub Copilot seat is licensed to
 * an individual, so one admin's token must not serve a whole team — each member
 * connects their own. The project only decides whether the agent is available
 * and which model it uses.
 *
 * Stores the long-lived GitHub OAuth token. The short-lived Copilot token
 * derived from it is never persisted: it expires in minutes, so caching it in
 * the database would mean writing on nearly every request for no benefit.
 */
export const llmCredentials = pgTable(
  "llm_credentials",
  {
    ...
    // GitHub login the token belongs to, for display and mismatch detection.
  ...
  (t) => ({
    // One credential per user per provider; reconnecting replaces it.
    userProviderUnique: uniqueIndex("llm_credentials_user_provider_unique").on(
      t.userId,
      t.provider
    ),
  })
