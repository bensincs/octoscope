...
export const projects = pgTable(
  "projects",
  {
    ...
    // Use each member's own GitHub sign-in instead of stored tokens.
    //
    // Implies localOnlyGithubData and is enforced as such: with per-member
    // tokens a SHARED cache would let someone read data GitHub would have
    // refused them, because visibility would depend on who refreshed last.
    // Isolating the data is the control, not a side effect.
    useViewerToken: boolean("use_viewer_token").notNull().default(false),
    // Data-residency control. When set, NOTHING fetched from GitHub is
    // persisted: issue snapshots, pull requests and decision records are
    // returned straight to the browser and kept in IndexedDB there instead.
    //
    // The server still fetches from GitHub — the PATs live server-side and
    // exposing them to the browser would be far worse — so the guarantee is
    // "never written to our database", not "never touches our servers".
    localOnlyGithubData: boolean("local_only_github_data")
      .notNull()
      .default(false),
    // Retention for GitHub-derived caches, in days. Null keeps data until it
    // is replaced by a refresh. Enforced two ways deliberately: reads ignore
    // anything past the limit so the policy is true immediately, and a
    // scheduled job deletes it so the policy is not merely cosmetic.
...
export const issueRepos = pgTable(
  "issue_repos",
  {
    ...
    name: text("name").notNull(),
    // Null when the project uses each member's GitHub sign-in instead.
 ...
 *
 * The token is stored as a SHA-256 hash, never in plaintext, and shown to the
 * creator once. A database leak therefore yields no usable invites — the same
 * reasoning as never returning a stored PAT.
 *
 * Redemption is a compare-and-set on `usedAt`, so two people opening the same
 * link cannot both be admitted.
 */
export const projectInvites = pgTable(
  "project_invites",
  {
    ...
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
      .notNull(),
    // Always set: an invite with no expiry is a permanent way into the project
  ...
  (t) => ({
    ...
    tokenUnique: uniqueIndex("project_invites_token_unique").on(t.tokenHash),
  })
);
