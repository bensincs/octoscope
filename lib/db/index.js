...
const pool =
  ...
  new Pool({
    ...
    // Enable TLS when the URL asks for it (managed Postgres in prod), and
    // actually VERIFY the server certificate.
    //
    // `rejectUnauthorized: false` gives encryption without authentication:
    // the connection is unreadable in transit but nothing proves the other end
    // is really our database, so it offers no protection against an active
    // attacker. Azure presents a chain rooted in DigiCert Global Root G2,
    // which Node already trusts, so verification needs no bundled CA file.
