...
describe("disabledRules", () => {
  const base = { levels: [["Epic"], ["Task"]] };

  it("defaults to an empty list", () => {
    const r = validateConfig(base);
    expect(r.ok).toBe(true);
    expect(r.value.disabledRules).toEqual([]);
  });

  it("accepts known rule ids", () => {
    const r = validateConfig({ ...base, disabledRules: ["orphan", "untyped"] });
    expect(r.ok).toBe(true);
    expect(r.value.disabledRules).toEqual(["orphan", "untyped"]);
  });

  // A typo that silently disables nothing would be worse than an error.
  it("rejects unknown rule ids", () => {
    const r = validateConfig({ ...base, disabledRules: ["nope"] });
    expect(r.ok).toBe(false);
    expect(r.errors.join(" ")).toMatch(/nope/);
  });

  it("de-duplicates", () => {
    const r = validateConfig({ ...base, disabledRules: ["orphan", "orphan"] });
    expect(r.value.disabledRules).toEqual(["orphan"]);
  });
});

describe("pullRequests rules", () => {
  const base = { levels: [["Epic"], ["Task"]] };

  // Configs written before PR rules existed must keep validating.
  it("defaults every flag on when absent", () => {
    const r = validateConfig(base);
    expect(r.value.pullRequests).toEqual({
  ...
  it("rejects unknown keys and non-booleans", () => {
    expect(validateConfig({ ...base, pullRequests: { nope: true } }).ok).toBe(false);
    expect(
      validateConfig({ ...base, pullRequests: { flagBehindBase: "yes" } }).ok
    ).toBe(false);
  });
});
