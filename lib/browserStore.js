...
async function withStore(mode, fn) {
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      // Resolve the REQUEST's result explicitly. An earlier version fell back
      // to the request object when the result was undefined, which meant a
      // cache miss returned an IDBRequest instead of nothing — truthy, so
      // callers treated it as stored data and read fields that did not exist.
      tx.oncomplete = () =>
...
export async function readLocal(projectId, feature) {
  try {
    const value = await withStore("readonly", (store) =>
      store.get(key(projectId, feature)),
    );
    // Only ever hand back something that looks like a stored payload. A
    // half-written or foreign value should read as "nothing cached" rather
    // than reaching a component that assumes its shape.
