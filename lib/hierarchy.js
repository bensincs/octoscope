...
//
// A "config" describes the issue-type hierarchy for one project:
//   {
...
export const DEFAULT_CONFIG = {
  ...
  allowedLabels: [],
  enforceLabels: false,
  // Pull-request health rules. Default on: each flags a state that already
  // blocks a merge, so surfacing them is not opinionated.
  pullRequests: {
...
/** Defaults applied when a config predates pull-request rules. */
export const DEFAULT_PR_RULES = { ...DEFAULT_CONFIG.pullRequests };
...
export function buildTree(issues, config = DEFAULT) {
  ...
  // Rules the project has switched off. Filtered after validation rather than
  // inside it so the checks stay pure and independent of configuration — a
  // disabled rule simply never reaches the UI.
  const disabled = new Set(config?.disabledRules ?? []);

  for (const issue of issues) {
    const problems = validateIssue(issue, H);
    byId.set(issue.id, {
      ...issue,
      level: H.levelOf(issue.type),
      children: [],
      problems: disabled.size
        ? problems.filter((p) => !disabled.has(p.code))
        : problems,
    });
  }
...
/** Every rule id, for validating `disabledRules` and rendering the rulebook. */
export const RULE_IDS = DEFAULT.rules.map((r) => r.id);
