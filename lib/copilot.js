...
//
//     This deployment sets it to Visual Studio Code's public client id — an
//     explicit, recorded decision by the project owner, not a default. The
//     editor headers below are matched to it deliberately: GitHub validates the
//     combination, so a mismatched client id and Editor-Version is rejected.
//     See infra/deploy.md for the reasoning and the risks accepted.
//
...
// Presented on every call. GitHub rejects the token exchange without them, and
// they must be consistent with COPILOT_CLIENT_ID — the service validates the
// combination, so a VS Code client id paired with a bespoke Editor-Version does
// not authenticate.
